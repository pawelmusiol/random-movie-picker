...
const getFilms = async (films) => {
    return Promise.all(films.map(async (film) => {
        let result = (await axios.get(`https://api.themoviedb.org/3/${film.type}/${film.id}?api_key=${process.env.TMDB_API_KEY}`)).data
        return {
            _id: film._id,
    ...
    switch (method) {
        case "GET":
            try {
                ...
                let FilmsData = await getFilms(result.films)
                res.send(FilmsData)
            } catch (err) {
